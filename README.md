...
│
├── models/
│   ├── Baseline_Models_Metrics_Evaluation.py
│   ├── HSD_Module.py
│   ├── LoBi_Codebase.py
│   └── AdaComp_Rag_Codebase.py
│
├── training/
│   ├── train_hola.py
│   ├── train_baseline.py
...
└── docs/
    └── figures/
        ├── hola_architecture.png
        ├── performance_comparison.png
        ├── cross_domain_heatmap.png
        ├── ablation_study.png
        ├── hardware_efficiency.png
        ├── tsne_domains.png
        └── ranking_shift.png
```
...
```bash
git clone https://github.com/YOUR_USERNAME/HOLA_Codebase.git
cd HOLA_Codebase
```
...
```bash
source hola-env/bin/activate
```

**Windows**

```bash
hola-env\Scripts\activate
```
...
For exact reproducibility, use the versions specified in `requirements.txt`.
...
```bash
python evaluation/evaluate_arc.py \
    --model_name TinyLlama/TinyLlama-1.1B-Chat-v1.0
```
...
```bash
python evaluation/cross_domain.py \
    --source gsm8k \
    --target arc
```

## Run hardware benchmarking
...
*Where supported by the hardware measurement setup.
...
**Hierarchical Selective Distillation**

File:

```text
models/HSD_Module.py
```

Purpose:

> Improve representation transfer and cross-domain generalization through selective intermediate-representation learning.

---

## Lo-Bi

**Low-Bitwidth Adaptation**

File:

```text
models/LoBi_Codebase.py
```

Purpose:

> Reduce memory requirements and facilitate inference on resource-constrained hardware.

---

## AdaComp-RAG

**Adaptive Compression with Retrieval-Augmented Generation**

File:

```text
models/AdaComp_Rag_Codebase.py
```

Purpose:

> Dynamically balance retrieval, compression, and generation to improve the efficiency of reasoning workloads.

---

# 🧪 Running the Visualization Suite

Generate the main plots with:

```bash
python visualization/plot_results.py
```

Generate the cross-domain heatmap:

```bash
python visualization/plot_heatmap.py
```

Generate t-SNE representations:
...
If you use this repository or HOLA in your research, please cite:

```bibtex
@article{hola2026,
  title={LLMs on a Budget: Say HOLA -- Efficient Instruction Tuning with Hierarchical Sparsity},
  author={YOUR NAME},
  journal={YOUR VENUE},
  year={2026}
}
...
> Replace the citation information with the final bibliographic details once the paper is published.
...
We thank the developers and researchers who make these tools publicly available.

---

# 📬 Contact

For questions, collaborations, or research discussions, please open an issue in this repository or contact the authors through the information provided in the associated paper.

---

<p align="center">

### ⭐ If you find HOLA useful, consider starring the repository!

**Efficient LLMs for everyone.**
